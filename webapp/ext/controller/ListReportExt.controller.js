sap.ui.controller("POWorklist.ext.controller.ListReportExt", {
	onInit: function(e)  {
		// /this.extensionAPI.refreshTable();
		//BuisnessPartner Model
		var sBusinessPartnersoModel = this.getOwnerComponent().getModel('BusinessPartnersModel');
		var sBusinessPartnersJSONModel;
		var that = this;
		sBusinessPartnersoModel.read("/BusinessPartners", {
			method: "GET",
		...
		//this.getView().setModel(sBusinessPartnersJSONModel,"bspartners");

		//ProductDetails Model
		var sProductDetailsoModel = this.getOwnerComponent().getModel('ProductDetailsModel');
		...
		//this.getView().setModel(sProductDetailsJSONModel,"products");
	...
	submit: function(oEvent) {
		...
		//	console.log(item);
		//	var KeyMapper =	"PURCHASEORDERID:300000000,PURCHASEORDERITEM:10,CURRENCY:EUR,GROSSAMOUNT:1000,NETAMOUNT:1000,TAXAMOUNT:100,QUANTITYUNIT:EA,DELIVERYDATE:2015-01-01";
		//	var KeyMapper = oBundle.getText("dummyValues");
		// var arrayMapper = KeyMapper.split(",");
		// for (var j = 0; j < arrayMapper.length; i++) {
		// 	var id1 = (arrayMapper[j].split(":"))[0];
		// 	var value1 = (arrayMapper[j].split(":"))[1];
		// 	item[id1] = value1;
		// }
		...
		$.ajax({
			type: "GET",
			async: false,
			url: "/POWorklist_fiori_elements/sap/hana/democontent/epm/services/poCreate.xsodata",
		...
		$.ajax({
			type: "POST",
			url: "/POWorklist_fiori_elements/sap/hana/democontent/epm/services/poCreate.xsodata/purchaseDetails",
			headers: {
	...
	new: function() {
		if (!this.dialog) {
			this.dialog = sap.ui.xmlfragment("POWorklist.ext.fragment.createPurchaseOrder", this);
	...
	deleteConfirm: function(bResult, oController, poId) {
		...
		if (bResult) {
			...
			var aUrl = '/POWorklist_fiori_elements/sap/hana/democontent/epm/services/poWorklistUpdate.xsjs?cmd=delete';
			jQuery.ajax({
				url: aUrl,
				method: 'POST',
				headers: {
	...
	exportExcel: function(oEvent) {
		window.open("/POWorklist_fiori_elements/sap/hana/democontent/epm/services/poWorklistQuery.xsjs?cmd=Excel");
		return;
	},
	exportZip: function(oEvent) {
		window.open("/POWorklist_fiori_elements/sap/hana/democontent/epm/services/poWorklistQuery.xsjs?cmd=Zip");
		return;
	},
	...
	approvalConfirm: function(bResult, oController, poId, action) {
		...
		if (bResult) {
			...
			var aUrl = '/POWorklist_fiori_elements/sap/hana/democontent/epm/services/poWorklistUpdate.xsjs?cmd=approval';
			jQuery.ajax({
				url: aUrl,
				headers: {
					'x-csrf-token': xsrf_token
	...
	onSuggest: function(oEvent) {
		...
		if (gSearchParam.length >= 3) {
			var aUrl = '/POWorklist_fiori_elements/sap/hana/democontent/epm/services/poWorklistQuery.xsjs?cmd=filter' + '&query=' + escape(gSearchParam) + '&page=1&start=0&limit=25';
			jQuery.ajax({
				url: aUrl,
				method: 'GET',
				dataType: 'json',
...
function onLoadFilter(myJSON,gSearchParam) {
	var oSearchControl = sap.ui.getCore().byId("POWorklist::sap.suite.ui.generic.template.ListReport.view.ListReport::POHeader--filterBy");
    var aSuggestions = [];
